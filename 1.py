...
background_img = pygame.image.load("G:/py class/pyproject/main_bg_12.jpg")
background_img = pygame.transform.scale(background_img, (screen_width, screen_height)).convert_alpha()

# background_vid = VideoFileClip("G:/py class/pyproject/forest.mp4")
...
def gameloop():
    ...
    play_time = 0
    level = 1
    score_thresholds = [0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1001]
    ...
    while not exit_game:
        if game_over:
            ...
            if not pause:  # Only update the game when it's not paused
                ...
                minutes = int(play_time / 60000)  # 1 minute = 60000 ms
                seconds = int((play_time % 60000) / 1000)
                ...
                if abs(snake_x - food_x) < 9 and abs(snake_y - food_y) < 9:
                    ...
                    if score > score_thresholds[level]:
                        level += 1
                        # init_velocity += 1  # Increase the game speed when reaching a new level
                        # You can adjust other game parameters for different levels here

                    # Check if the player has reached the maximum level
                    if level > len(score_thresholds) - 1:
                        level = len(score_thresholds) - 1

                gameWindow.fill(black)
                gameWindow.blit(background_img, (0, 0))
                text_screen("Your Score is: " + str(score), green, 15, 1)
                text_screen("Level: " + str(level), blue, 15, 20)
                text_screen("The High Score is: " + str(high_score), red, 975, 1)
                text_screen("Press Space Bar to pause the game", green, 585, 1)
                text_screen(f"Playtime: {minutes} minute(s) {seconds} second(s)", red, 225, 1)
                # text_screen("Playtime: " + str(play_time % 60) + " seconds", red, 500, 35)
